...
from tactical_models import PRIORIDADES, ROSTER
...
class AIController:
    ...
    def elegir(actor, aliados, jefe, escudo, escudo_activado=False):
        ...
        for regla in PRIORIDADES[actor.seleccion.prioridad]["reglas"]:
            if regla == "limpiar" and actor.seleccion.build == "adaptacion" and disponible("limpiar"):
                afectados = vivos if actor.id == "cora" else [actor]
            ...
            elif regla == "curar" and actor.id == "cora" and disponible("curar"):
                ...
                if actor.seleccion.arma == "maza_hierro" or actor.id == "bruno":
                    return Accion("romper", jefe.id, regla)
            elif regla == "unica" and disponible("unica"):
                # Táctica reserva el recurso del guerrero hasta la ventana del escudo.
                if actor.id == "bruno" and actor.seleccion.prioridad == "tactica" and not escudo_activado:
                    continue
                return Accion(ROSTER[actor.id]["unica"], actor.id if actor.id == "cora" else jefe.id, regla)
