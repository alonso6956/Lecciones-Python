...
def tirar_dano(modelo, rng):
    return modelo.calcular_dano_base() + obtener_dano_arma(modelo.arma, rng)
...
def estadisticas_combate(modelo):
    ...
    datos = {
        ...
        "ataque_minimo": base + arma.ataque[0],
        "ataque_maximo": base + arma.ataque[1],
        ...
        "probabilidad_bloqueo": secundario.probabilidad_bloqueo if secundario else 0,
        "porcentaje_dano_bloqueado": secundario.porcentaje_dano_bloqueado if secundario else 0,
    }
    if jugador:
